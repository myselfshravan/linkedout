(() => {
  ...
  async function extractProfileData() {
    try {
      const profile = {
        name: document.querySelector('h1.text-heading-xlarge')?.innerText?.trim(),
        headline: document.querySelector('div.text-body-medium')?.innerText?.trim(),
        about: document.querySelector('div#about ~ div.display-flex div.pv-shared-text-with-see-more')?.innerText?.trim(),
        experience: Array.from(document.querySelectorAll('div#experience ~ div ul.pvs-list > li')).map(exp => ({
          title: exp.querySelector('.t-bold span')?.innerText?.trim(),
          company: exp.querySelector('.t-normal span')?.innerText?.trim(),
          duration: exp.querySelector('.t-normal.t-black--light span')?.innerText?.trim()
        })),
        education: Array.from(document.querySelectorAll('div#education ~ div ul.pvs-list > li')).map(edu => ({
          school: edu.querySelector('.t-bold span')?.innerText?.trim(),
          degree: edu.querySelector('.t-normal span')?.innerText?.trim(),
          duration: edu.querySelector('.t-normal.t-black--light span')?.innerText?.trim()
        })),
        skills: Array.from(document.querySelectorAll('div#skills ~ div ul.pvs-list > li')).map(skill => 
